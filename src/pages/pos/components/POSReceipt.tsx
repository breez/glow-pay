...
export function POSReceipt({ amountSats, paymentId, items, storeName, rate, onNewSale }: POSReceiptProps) {
  ...
  const usdAmount = rate ? satsToUsd(amountSats, rate) : 0
  const showUsd = rate && usdAmount >= 0.01

  return (
    <div className="fixed inset-0 gradient-bg z-40 flex flex-col items-center p-6 overflow-y-auto">
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-sm">
        ...
        <p className="text-3xl font-bold tracking-tight tabular-nums text-white mb-8">
          {formatSats(amountSats)} sats
        </p>

        {/* Printable receipt */}
        <div className="pos-receipt-print w-full">
      ...
      {/* Screen buttons — pinned to bottom */}
      <div className="flex gap-3 no-print pt-6 pb-2 w-full max-w-sm">
        <button
          onClick={() => window.print()}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-surface-800/60 border border-white/[0.06] rounded-xl text-sm text-gray-300 hover:text-white transition-colors"
        >
        ...
        <button
          onClick={onNewSale}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-glow-400 hover:bg-glow-300 active:bg-glow-500 text-surface-900 font-bold rounded-xl transition-colors text-sm"
        >
